...
def go(config: DictConfig):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if "basic_cleaning" in active_steps:
            mlflow.run(
                ...
                parameters={
                    ...
                    "output_description": (
                        "Data after basic price filtering, date casting, "
                        "and NYC-bounding‐box clipping"
                    ),
                    "min_price": config["etl"]["min_price"],    # 10
                    "max_price": config["etl"]["max_price"],    # 350
                },
            )
        ...
        if "train_random_forest" in active_steps:
            ...
            # ── dump current RF hyper-params to JSON so the step can read them
